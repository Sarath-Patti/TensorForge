...
class TestParallelKernels(unittest.TestCase):

    def setUp(self):
        if not is_native_available():
            self.skipTest("Native C++ extension not compiled")

    def test_parallel_matmul_parity(self):
        np.random.seed(42)
        m, k, n = 64, 128, 64
    ...
    def test_parallel_fused_linear_relu_parity(self):
        np.random.seed(42)
        m, k, n = 64, 128, 32
    ...
    def test_parallel_fused_linear_activations_parity(self):
        np.random.seed(42)
        m, k, n = 32, 64, 16
        x = np.random.randn(m, k).astype(np.float32)
        w = np.random.randn(n, k).astype(np.float32)
    ...
    def test_parallel_fused_qlinear_relu_parity(self):
        np.random.seed(42)
        m, k, n = 64, 128, 32
        x_t = tf.randn((m, k))
        w_t = tf.randn((n, k))
